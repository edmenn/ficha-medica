...
export async function GET(req: NextRequest) {
  ...
  const limiter = rateLimit(`export:${clientIp(req)}:${ctx.effectiveUserId}`, { limit: 30, windowMs: 60 * 1000 })
  if (!limiter.allowed) {
    return NextResponse.json({ error: 'Demasiadas exportaciones. Intentá de nuevo en un momento.' }, { status: 429 })
  }
