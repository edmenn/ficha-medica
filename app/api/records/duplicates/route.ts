...
export async function POST(req: NextRequest) {
  ...
  if (!normalized.paciente || !normalized.fecha_cirugia) {
    return NextResponse.json({ existing_id: null, score: 0 })
  }
  ...
  const duplicate = findDuplicate(data ?? [], normalized, body.exclude_record_id)
  return NextResponse.json({
    existing_id: duplicate?.existing_id ?? null,
